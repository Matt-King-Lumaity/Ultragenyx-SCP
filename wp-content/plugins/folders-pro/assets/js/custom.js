			...
			jQuery.ajax({
				...
				success: function (res) {
					...
					if (result.status == '1') {
                        ...
                        jQuery(".wcp_folder_"+result.id+" span.folder").text(result.term_title);

                        if(jQuery(".sticky-folders .sticky-folder-"+fileFolderID).length) {
                            jQuery(".sticky-folders .sticky-folder-"+fileFolderID+" .folder-title").text(result.term_title);
                        }
			...
			jQuery.ajax({
				...
				success: function (res) {
					...
					if (res.status == '1') {
                        ...
                        jQuery(".sticky-folders .sticky-folder-"+fileFolderID).remove();
						add_menu_to_list();
						ajaxAnimation();
						jQuery(".folder-popup-form").hide();
						jQuery(".folder-popup-form").removeClass("disabled");
...
jQuery(document).ready(function(){
    ...
    jQuery(document).on("click", ".sticky-folders a", function(e) {
        e.stopPropagation();
        var folder_ID = jQuery(this).closest("li").data("folder-id");
        if(jQuery("#wcp_folder_"+folder_ID).length) {
            jQuery("#wcp_folder_"+folder_ID+" > h3.title").trigger("click");
        }
    });
    ...
    jQuery(".sticky-folders li a").livequery(function(){
        jQuery(this).droppable({
            accept: ".wcp-move-file, .wcp-move-multiple, .attachments-browser li.attachment",
            hoverClass: 'wcp-drop-hover',
            classes: {
            ...
            drop: function( event, ui ) {
                folderID = jQuery(this).closest("li").data('folder-id');
                if ( ui.draggable.hasClass( 'wcp-move-multiple' ) ) {
                    if(jQuery(".wp-list-table input:checked").length) {
                        chkStr = "";
                        jQuery(".wp-list-table input:checked").each(function(){
	...
	jQuery("h3.title").livequery(function(){
		jQuery(this).on("contextmenu",function(e) {
            ...
            isSticky = jQuery(this).closest("li.route").hasClass("is-sticky");
			...
			menuHtml = "<div class='dynamic-menu'><ul>" +
				...
				"<li class='sticky-folder'><a href='javascript:;'><span class='sticky-pin'><i class='pfolder-pin'></i></span>" + ((isStickyClass) ? "Remove Sticky Folder" : "Sticky Folder") + "</a></li>" +
	...
	jQuery(".rename-folder").livequery(function(){
		jQuery(this).click(function(e){
			e.stopPropagation();
            if(!jQuery(this).closest(".sticky-folders").length) {
                fileFolderID = jQuery(this).closest("li.route").data("folder-id");
            } else {
                fileFolderID = jQuery(this).closest("li.sticky-fldr").data("folder-id");
            }
			updateFolder();
		});
	});

	jQuery(".sticky-folder").livequery(function(){
		jQuery(this).click(function(e){
			e.stopPropagation();
            if(!jQuery(this).closest(".sticky-folders").length) {
                fileFolderID = jQuery(this).closest("li.route").data("folder-id");
            ...
            var folder_status = 0;
			if(!jQuery("#wcp_folder_"+fileFolderID).hasClass("is-sticky")) {
                folder_status = 1;
                jQuery("#wcp_folder_"+fileFolderID).addClass("is-sticky");
            } else {
                jQuery("#wcp_folder_"+fileFolderID).removeClass("is-sticky");
                jQuery(".sticky-folders .sticky-folder-"+fileFolderID).remove();
            }
            jQuery(".dynamic-menu").hide();
            nonce = jQuery.trim(jQuery("#wcp_folder_"+fileFolderID).data("star"));
            make_sticky_folder_menu();
            jQuery.ajax({
                url: wcp_settings.ajax_url,
                data: "term_id=" + fileFolderID + "&type=" + wcp_settings.post_type + "&action=wcp_make_sticky_folder&nonce="+nonce+"&status="+folder_status,
                method: 'post',
                cache: false,
                success: function (res) {
    ...
    jQuery(".download-folder").livequery(function(){
        jQuery(this).click(function(){
            if(!jQuery(this).closest(".sticky-folders").length) {
                fileFolderID = jQuery(this).closest("li.route").data("folder-id");
            } else {
            ...
            dataNonce = jQuery("#wcp_folder_"+fileFolderID).attr("data-nonce");
            window.location = wcp_settings.admin_url+"?action=download-folders&folder_id="+fileFolderID+"&nonce="+dataNonce;
        });
    });
	...
	jQuery(".mark-folder").livequery(function(){
		jQuery(this).click(function(e){
            ...
            if(!jQuery(this).closest(".sticky-folders").length) {
                ...
                folderID = jQuery(this).closest("li.sticky-fldr").data("folder-id");
            }
				...
				success: function (res) {
					...
					if (res.status == '1') {
						if(res.marked == '1') {
							...
							jQuery(".sticky-folder-"+res.id).addClass("is-high");
						} else {
							jQuery("#wcp_folder_"+res.id).removeClass("is-high");
							jQuery(".sticky-folder-"+res.id).removeClass("is-high");
						}
						add_menu_to_list();
						ajaxAnimation();
					} else {
	...
	jQuery(".update-inline-record").livequery(function(){
		jQuery(this).click(function(e){
            ...
            if(!jQuery(this).closest(".sticky-folders").length) {
                isHigh = jQuery(this).closest("li.route").hasClass("is-high");
                isStickyClass = jQuery(this).closest("li.route").hasClass("is-sticky");
                jQuery(".dynamic-menu").remove();
                jQuery(".active-menu").removeClass("active-menu");
				menuHtml = "<div class='dynamic-menu'><ul>" +
					"<li class='new-folder'><a href='javascript:;'><span class=''><i class='pfolder-add-folder'></i></span> New Folder</a></li>" +
                ...
                jQuery(".dynamic-menu").remove();
                isHigh = jQuery(this).closest("li.sticky-fldr").hasClass("is-high");
                isStickyClass = true;
                menuHtml = "<div class='dynamic-menu'><ul>" +
                ...
                "<li class='sticky-folder'><a href='javascript:;'><span class='sticky-folder-icon'><i class='pfolder-pin'></i></span>" + ((isStickyClass) ? "Remove Sticky Folder" : "Sticky Folder") + "</a></li>" +
                "<li class='mark-folder'><a href='javascript:;'><span class='folder-icon-star_rate'></span>" + ((isHigh) ? " Remove Star" : "Add a Star") + "</a></li>";
                hasPosts = parseInt(jQuery(this).closest("li.sticky-fldr").find(".folder-count").text());
                if (wcp_settings.post_type == "attachment" && hasPosts) {
                    menuHtml += "<li class='download-folder'><a href='javascript:;'><span class='dashicons dashicons-media-archive'></span> Download Zip</a></li>";
                }
                menuHtml += "<li class='remove-folder'><a href='javascript:;'><span class='folder-icon-delete'></span> Delete</a></li>" +
                "</ul></div>";
                jQuery(this).closest("li").append(menuHtml);
                jQuery(this).closest("li").addClass("active-menu");

                if((jQuery(this).closest("li").offset().top + jQuery(".dynamic-menu").height()) > (jQuery(window).height() - 20)) {
                    jQuery(".dynamic-menu").addClass("bottom-fix");
...
function make_sticky_folder_menu() {
    jQuery(".sticky-folders > ul").html("");
    var stickyMenuHtml = "";

    jQuery("#custom-menu li.route.is-sticky").each(function(){
        var folder_ID = jQuery(this).data("folder-id");
        var folderName = jQuery("#wcp_folder_"+folder_ID+" > h3 > .title-text").text();
        var folderCount = jQuery("#wcp_folder_"+folder_ID+" > h3 > .total-count").text();
        var hasStar = jQuery("#wcp_folder_"+folder_ID).hasClass("is-high")?" is-high ":"";
    ...
    jQuery(".sticky-folders > ul").html(stickyMenuHtml);
    if(jQuery("#custom-menu li.route.active-item").length) {
        var activeTermId = jQuery("#custom-menu li.route.active-item").data("folder-id");
        jQuery(".sticky-folders .sticky-folder-"+activeTermId+" a").addClass("active-item");
    }

    if(jQuery(".sticky-folders > ul > li").length > 0) {
        jQuery(".sticky-folders").addClass("active");
    } else {
        jQuery(".sticky-folders").removeClass("active");
    }
