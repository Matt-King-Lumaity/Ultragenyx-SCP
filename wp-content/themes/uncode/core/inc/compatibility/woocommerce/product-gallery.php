<?php
/**
 * Product Gallery (WC default) functions
 */
...
/**
 * Load default WC product gallery templates from WooCommerce folder
 */
...
/**
 * Get default WC product gallery settings from Theme Options and Page Options
 */
function uncode_woocommerce_get_default_product_gallery_settings() {
	static $settings = '';
	...
	$generic_layout     = $generic_layout === 'stack' ? 'stack' : 'standard';
	$generic_columns    = absint( ot_get_option( '_uncode_product_thumb_cols' ) );
	$generic_columns    = $generic_columns ? $generic_columns : 3;
	$generic_columns    = $generic_layout === 'stack' ? 1 : $generic_columns;
	$generic_zoom       = $generic_layout === 'stack' ? false : ot_get_option( '_uncode_product_enable_zoom' );
	$generic_zoom       = $generic_zoom === 'on' ? true : false;
	$generic_flexslider = $generic_layout === 'stack' ? false : true;
	$generic_lightbox   = $generic_layout === 'stack' ? false : true;
	...
	if ( $post_id > 0 ) {
							...
							foreach ( $product_gallery_atts as $key_attr => $value_attr ) {
								if ( isset( $value_attr[1] ) && isset( $value_attr[2] ) ) {
									switch ( trim( $value_attr[1] ) ) {
										case 'layout':
											$module_layout = $value_attr[2] === 'stack' ? 'stack' : $module_layout;
											break;
		...
		if ( $module_found ) {
			$module_layout     = $module_layout === 'stack' ? 'stack' : 'standard';
			$module_columns    = $module_columns ? $module_columns : 3;
			$module_columns    = $module_layout === 'stack' ? 1 : $module_columns;
			$module_zoom       = $module_layout === 'stack' ? false : $module_zoom;
			$module_flexslider = $module_layout === 'stack' ? false : true;
			$module_lightbox   = $module_layout === 'stack' ? false : true;
			...
			// Flexslider
			$specific_flexslider = $specific_layout === 'stack' ? false : true;

			// Lightbox
			$specific_lightbox = $specific_layout === 'stack' ? false : true;
...
/**
 * Change default WC product gallery columns
 */
function uncode_woocommerce_default_product_gallery_thumbnails_columns( $columns ) {
	if ( ot_get_option('_uncode_woocommerce_default_product_gallery') === 'on' ) {
		$settings = uncode_woocommerce_get_default_product_gallery_settings();
...
/**
 * Enable/disable default WC product gallery zoom
 */
...
/**
 * Enable/disable default WC product gallery flexslider
 */
...
/**
 * Change default product gallery columns (1 column)
 */
...
/**
 * Change default product gallery columns (2 columns)
 */
...
/**
 * Change default product gallery columns (3 columns)
 */
...
/**
 * Change default product gallery columns (4 columns)
 */
...
/**
 * Change default product gallery columns (5 columns)
 */
...
/**
 * Change default product gallery columns (6 columns)
 */
